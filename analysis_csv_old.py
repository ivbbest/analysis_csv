"""Анализ csv файла и для контракта, кд, договора вытаскивать максимум информации, включая дату и тд."""
...
contract_pattern_main = "((дог|кд|к\/д|догN|КД|КД_|к\.д|к\д)?\s+(№|N)?\s?(\d){2,}\s+(от\s?(\w{2}(\.|\/|\,)){2}\w{2,4}))"
# паттерн фио
fio_pattern = "([А-ЯЁ][А-Яа-яёЁ]+\s{1,2}?([А-ЯЁ][А-Яа-яёЁ]+\s?|[А-ЯЁ]+\.|[А-ЯЁ]+\,|[А-ЯЁ]+|[А-ЯЁ]+\/){2})"
# второй паттерн работает для отдельных случаев, чтобы добавить больше договор в итоговый файл
contract_pattern_other = "(дог|кд|к\/д|догN|КД|КД_|к\.д|к\д|N|№|Дог\.|кд\.|Дог\.№|дог\.№|КД\s+№)([\s|\d|\.|\-|от|ОЦ|НД|/|_|N|Z|PHA|УПД]{3,})"


def analysis_csv_file(in_file):
    """Анализ csv файла и запись данных в списки: фио, контракт и общие данные"""
    # объявляем списки, куда будем записывать все фио, контракты и все считанные данные
    all_fio = list()
    all_data = list()
    all_contract = list()

    # считываем файл
    df = pd.read_csv(in_file, sep=';', encoding='cp1251', keep_default_na=False)
    ...
    for line in df['comments']:
        ...
        # если для контракта не срабатывает первый паттерн для анализа, то использую второй
        try:
            if len(contract) == 0:
                ...
                contract_text = '; '.join(map(str, [c[0].strip() for c in contract])).strip()
