...
contract_pattern = "((дог|кд|к\/д|догN|КД|КД_|к\.д|к\д)?\s+(№|N)?\s?(\d){2,}\s+(от\s?(\w{2}(\.|\/|\,)){2}\w{2,4}))"
# паттерн фио
...
contract_pattern2 = "(дог|кд|к\/д|догN|КД|КД_|к\.д|к\д|N|№|Дог\.|кд\.|Дог\.№|дог\.№|КД\s+№)([\s|\d|\.|\-|от|ОЦ|НД|/|_|N|Z|PHA|УПД]{3,})"
...
def analysis_csv_file(in_file):
    ...
    for line in df['comments']:
        ...
        try:
            if len(contract) == 0:
                contract = re.findall(contract_pattern2, str(line))
                print(contract)
