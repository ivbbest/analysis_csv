"""Анализ csv файла и для контракта, кд, договора вытаскивать только цифры с разбиением по столбцам"""
...
# паттерн для финального вытаскивания только цифр кд,договора или контракта без дат
contract_pattern = "(\d{3,})"

# года для исключения в финальном паттерне
years = set([year for year in range(2000, 2023, 1)])
...
def analysis_csv_file(in_file):
    ...
    for line in df['comments']:
        try:
            ...
            contract_text = ';'.join(map(str, [c.strip() for c in contract if int(c) not in years]))
            print(contract_text)
